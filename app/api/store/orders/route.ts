...
/**
 * Storefront order history for the LOGGED-IN customer only.
 *
 * A shopper can only ever see orders for the phone number they verified via OTP (read from the
 * signed httpOnly session cookie) — never by typing an arbitrary phone. Runs server-side with the
 * service role so it keeps working once `customers`/`sales` are locked down by RLS.
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: sales } = await supabase
      .from("sales")
      .select("*, customer:customers(*), items:sale_items(*, product:products(name, image_url))")
      .eq("shop_id", shopId)
      .in("customer_id", customerIds)
